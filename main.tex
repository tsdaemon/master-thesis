...
\emph{Author:}\\
Anatolii Stehnii{\authorname}
\end{flushleft}
\end{minipage}
\begin{minipage}[t]{0.4\textwidth}
\begin{flushright} \large
\emph{Supervisor:} \\
Dr. Rostyslav Hryniv{\supname} % Supervisor name - remove the \href bracket to remove the link  
\end{flushright}
\end{minipage}\\[3cm]
...
\textbf{AST} & \textbf{A}abstract \textbf{S}yntax \textbf{T}ree\\
...
$v_word$ & word embedding vector

\addlinespace 

`embed` & word embedding operation  \\
`softmax` & normalized exponential function \\

\addlinespace 

$\alpha$ & Attention vector & \si{\radian} \\
