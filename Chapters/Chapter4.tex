...
Let us consider as example the AST from Fig. \ref{fig:ast}. Its generation consist of the following actions:
...
For each time step $t$ the model selects the next action with a maximum probability --- \code{ApplyRule} to grow the tree or \code{GenToken} to fill values in terminal nodes. 
