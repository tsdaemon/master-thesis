...
\begin{figure}
\centering
\includegraphics{Figures/dependency_graph}
\decoRule
\caption[Syntax tree]{Dependency graph for sentence \textquote{nice dogs like cats}\footnote{Picture is created with tool \code{GrammarScope}\href{http://grammarscope.sourceforge.net/}}.}
\label{fig:dependency_graph}
\end{figure}

In Fig \ref{fig:dependepncy_parse} sentence \textquote{nice dogs like cats} parsed to dependency graph. It consists of following dependencies: 
...
Neural network operates with vector representations of words which selected from predefined vocabulary. This imposes the problem of unknown words which don't have vector representation. This is especially important for the translation task where both input and output sequences could contain rare, special words or names. However, names of people or locations should not be translated but copied to target sequence. \cite{Vinyals2015} proposed solution of this problem with \emph{pointer network}. When decoding finished it calculates the probability of next word to be copied from an input sequence. Calculation of this probability step-by-step described below.

Let's denote $H_{encoder}$ as a matrix of input sequence vectors and $h_{decoder}^{(t)}$ - decoder hidden state vector on encoding step $t$. First is calculated hidden state of pointer network:

\begin{equation}
    \begin{gathered}
    
    H_{encoder_pointer} = H_{encoder} \cdot W_{encoder}\\
    
    h_{decoder_pointer} = h_{decoder}^{(t)} \cdot W_{decoder}\\
    
    H_{pointer} = tanh(H_{encoder_pointer} + h_{decoder_pointer})\\
