...
\section{Sequence-to-sequence machine translation with attention} \label{attention}
...
\begin{equation}
h_d^{(t)} = lstm([w^{(t)}, \phi^{(t-1)}], h_d^{(t-1)})
\label{attn:hd}
\end{equation}
...
\begin{equation}
\begin{gathered}
\hat{\alpha}_i^{(t-1)} = W_{attn1} \cdot [h_e^{(i)}, h_d^{(t-1)}] \\
\alpha_i^{(t-1)} = W_{attn2} \cdot tanh(\hat{\alpha}_{i}^{(t-1)}) \\
\alpha^{(t-1)} = softmax(\alpha^{(t-1)})
\end{gathered}
\label{attn:alpha}
\end{equation}
...
Vector $P$ contains probabilities values for each input sequence token to be copied into output sequence.
